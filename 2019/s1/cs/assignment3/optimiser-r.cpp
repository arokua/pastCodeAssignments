...
ast walk_while(ast t)
{
    ...
    //Get the first expression node from condition
    ast temp = get_expr(condition, 0);
    if (ast_node_kind(get_term_term(temp)) == ast_bool ) {
    	//Check if the condition is true
    	if (get_bool_t_or_f(get_term_term(temp)) == true) {
